...
fleet <- read_csv("http://www.massey.ac.nz/~jcmarsha/161122/data/fleet.csv.gz")
...
```{r}
colours_by_year <- cars_vans %>% group_by(VEHICLE_YEAR) %>%
  count(BASIC_COLOUR) %>%
...
ggplot(data = colours_by_year) +
  geom_line(mapping = aes(x = VEHICLE_YEAR, y=Proportion)) +
  facet_wrap(vars(BASIC_COLOUR)) +
  labs(title = "The rise of orange and demise of brown and gold",
...
prop_utes %>% filter(Type == "Ranger") %>%
  ggplot() +
  geom_col(mapping = aes(y=fct_reorder(TLA, Proportion), x=Proportion)) +
  labs(y = NULL,
       x = "Percentage of Ford rangers",
       title = "Are Ford Rangers really a Rural thing?") +
...
My conclusion is that Ford Rangers really aren't a rural thing at all. Lots of them are owned by Urban people. Some of these might be for a good reason! e.g. Tradies, but a bunch are probably just because they look cool (or people think they look cool!)
...
We can figure this out using `MAKE`, `VEHICLE_YEAR`, `BODY_TYPE`, `BASIC_COLOUR`, `TLA` and `NUMBER_OF_SEATS`,
