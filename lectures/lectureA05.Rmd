...
Prior to R version 4.1, the `|>` (base) pipe didn't exist. Instead, people used a pipe from the `magittr` package which uses `%>%`.

Generally, these are basically the same. You will see `%>%` used 'in the wild' when googling - that's OK, it'll work just fine.

In RStudio, by default the keyboard combination Ctrl-Shift-M (Cmd-Shift-M on Mac) inserts the magittr pipe `%>%`.
